...
public class AmethystNecklace extends Item implements ICurioItem,ManaIncreasingItem {
    private static final int MAX_MANA_INCREASE = 50; // Увеличение максимальной маны
    private static final int MINIMUM_MANA_LEVEL = 1;  // Минимально допустимый уровень маны

    public AmethystNecklace() {
    ...
    public void onEquip(SlotContext slotContext, ItemStack prevStack, ItemStack stack) {
        ...
        if (isMage(player)) {
            increaseMana(player);
            player.sendSystemMessage(Component.translatable("sbm.curios.amethyst_necklace.dressed").withStyle(ChatFormatting.LIGHT_PURPLE));
        }
        else{player.sendSystemMessage(Component.translatable("sbm.curios.amethyst_necklace.warn").withStyle(ChatFormatting.RED));}
    }
    ...
    public void onUnequip(SlotContext slotContext, ItemStack newStack, ItemStack stack) {
        Player player = (Player) slotContext.entity();
        decreaseMana(player);
        player.sendSystemMessage(Component.translatable("sbm.curios.amethyst_necklace.undressed").withStyle(ChatFormatting.LIGHT_PURPLE));
    }
